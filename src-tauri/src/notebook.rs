...
fn spawn(mut command: Command) -> Result<Child, String> {
    ...
    command
        .spawn()
        .map_err(|e| format!("Could not start the bundled Notebook service: {e}"))
}
...
fn copy_tree(source: &Path, destination: &Path) -> Result<(), String> {
    ...
    for entry in fs::read_dir(source).map_err(|e| e.to_string())? {
        ...
        if kind.is_symlink() {
            return Err("Notebook backup refuses symbolic links in its data directory".into());
        }
        let out = destination.join(entry.file_name());
        if kind.is_dir() {
            copy_tree(&entry.path(), &out)?;
        } else {
...
async fn ready(runtime: &mut Runtime, url: &str, phase: &str) -> Result<(), String> {
    ...
    while Instant::now() < deadline {
        if !runtime.alive() {
            return Err(format!(
                "Notebook {phase} exited during startup. Retry or reinstall the bundled runtime."
            ));
        }
    ...
    Err(format!(
        "Notebook {phase} did not become ready within two minutes."
    ))
}
...
async fn stop_locked(state: &NotebookState) {
    ...
    if let Some(runtime) = runtime {
        ...
        let _ = c
            .client
            .post(format!("{}/api/prism/drain", c.base_url))
            .bearer_auth(&c.password)
            .timeout(Duration::from_secs(2))
            .send()
            .await;
        ...
        while Instant::now() < deadline {
            let jobs = c
                .client
                .get(format!(
                    "{}/api/commands/jobs?status_filter=running&limit=100",
                    c.base_url
                ))
                .bearer_auth(&c.password)
                .timeout(Duration::from_secs(2))
            ...
            if idle && c.inflight.load(Ordering::SeqCst) == 0 {
                break;
            }
            tokio::time::sleep(Duration::from_millis(250)).await;
        }
...
#[tauri::command]
pub async fn notebook_stop(state: State<'_, NotebookState>) -> Result<(), String> {
    ...
    Ok(match guard.as_mut() {
        Some(r) => RuntimeStatus {
            ...
            message: if r.alive() {
                ...
                Some("A Notebook service stopped. Restart Notebook to recover.".into())
            },
        },
        None => RuntimeStatus {
            state: "stopped".into(),
...
pub async fn notebook_start(
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        if major < 14 {
            return Err(
                "Notebook requires macOS 14 or newer. Prism's other pages remain available.".into(),
            );
        }
    }
    let vault = fs::canonicalize(vault_path)
        .map_err(|_| "Select an existing vault before opening Notebook".to_string())?;
    if !vault.is_dir() {
        return Err("Notebook requires a vault folder".into());
    }
    {
        let mut guard = state.runtime.lock().await;
    ...
    let manifest: Manifest =
        serde_json::from_slice(&fs::read(root.join("manifest.json")).map_err(|e| e.to_string())?)
            .map_err(|e| e.to_string())?;
    if manifest.knowledge_gateway != 1 || !root.join("prism_gateway.py").exists() {
        return Err("Notebook runtime needs the Prism knowledge gateway update. Rebuild or update the bundled runtime.".into());
    }
    let workspace = vault.join(".prism/notebook");
    fs::create_dir_all(&workspace).map_err(|e| e.to_string())?;
    let workspace = fs::canonicalize(workspace).map_err(|e| e.to_string())?;
    if !workspace.starts_with(&vault) {
        return Err("Notebook data directory must remain inside the vault".into());
    }
    let workspace_lock = fs::OpenOptions::new()
        ...
        .read(true)
        .write(true)
        .open(workspace.join("runtime.lock"))
        .map_err(|e| e.to_string())?;
    workspace_lock.try_lock_exclusive().map_err(|_| {
        "This Notebook workspace is already open in another Prism process".to_string()
    })?;
    let identity_file = workspace.join("workspace-id");
    let identity = if identity_file.exists() {
        fs::read_to_string(&identity_file).map_err(|e| e.to_string())?
    } else {
        let id = Uuid::new_v4().to_string();
        fs::write(&identity_file, &id).map_err(|e| e.to_string())?;
        id
    };
    ...
    if changing {
        ...
        fs::write(
            &migration_pending,
            native_path(&backup).to_string_lossy().as_bytes(),
        )
        .map_err(|e| e.to_string())?;
    }
    ...
    .env("SURREAL_PASS", &database_password)
    .arg(format!("rocksdb:{}", data.join("surreal.db").display()));
    runtime.children.push(spawn(db)?);
    ready(
        &mut runtime,
        &format!("http://127.0.0.1:{db_port}/health"),
        "database",
    )
    .await?;
    ...
    for service in ["api", "worker"] {
        ...
        command
            .arg("-B")
            .arg(root.join("launcher.py"))
            .arg(service)
            .arg(api_port.to_string())
            .env("PRISM_NOTEBOOK_DATA", &data)
            .env("SURREAL_URL", format!("ws://127.0.0.1:{db_port}/rpc"))
            .env("SURREAL_USER", "prism")
            .env("SURREAL_PASSWORD", &database_password)
            .env("SURREAL_NAMESPACE", "open_notebook")
            .env("SURREAL_DATABASE", "open_notebook")
            .env("OPEN_NOTEBOOK_ENCRYPTION_KEY", &secret)
            .env("OPEN_NOTEBOOK_PASSWORD", &password)
    ...
    if !runtime.alive() {
        return Err("Notebook worker could not start".into());
    }
    fs::write(&version_file, &manifest.revision).map_err(|e| e.to_string())?;
    if changing {
        fs::remove_file(migration_pending).map_err(|e| e.to_string())?;
    }
    let status = RuntimeStatus {
        state: "ready".into(),
        workspace_id: Some(runtime.connection.id.clone()),
        vault_path: Some(vault.to_string_lossy().into()),
        message: None,
    };
...
async fn lease(state: &NotebookState, workspace_id: &str) -> Result<Lease, String> {
    ...
    if r.connection.id != workspace_id || !r.connection.accepting.load(Ordering::SeqCst) {
        return Err("Notebook workspace changed. Reopen the current notebook.".into());
    }
    if !r.alive() {
        return Err("A Notebook service stopped. Restart Notebook.".into());
    }
...
async fn response_value(response: reqwest::Response) -> Result<Value, String> {
    ...
    let text = response
        .text()
        .await
        .map_err(|_| "Notebook response was interrupted".to_string())?;
    if !status.is_success() {
        let detail = serde_json::from_str::<Value>(&text)
            .ok()
            .and_then(|v| v.get("detail").cloned())
            .unwrap_or(json!("Notebook request failed"));
...
pub async fn notebook_request(
    ...
    if !["GET", "POST", "PUT", "DELETE", "PATCH"].contains(&method.as_str()) {
        return Err("Unsupported Notebook method".into());
    }
    ...
    response_value(
        request.send().await.map_err(|_| {
            "Could not reach Notebook. Check its runtime status and retry.".to_string()
        })?,
    )
    .await
}
...
#[tauri::command]
pub async fn notebook_add_source(
    state: State<'_, NotebookState>,
    workspace_id: String,
    ...
    if upload {
        ...
        if size > 95 * 1024 * 1024 {
            return Err("Choose a file smaller than 95 MB".into());
        }
        form = form.part(
            "file",
            reqwest::multipart::Part::bytes(file.read().await).file_name(file.file_name()),
        );
    }
    if !c.accepting.load(Ordering::SeqCst) {
        return Err("Notebook workspace changed".into());
    }
    response_value(
        c.client
            .post(format!("{}/api/sources", c.base_url))
            .bearer_auth(&c.password)
            .multipart(form)
            .send()
            .await
            .map_err(|_| "Source upload failed".to_string())?,
    )
    .await
}

#[tauri::command]
pub async fn notebook_media(
    state: State<'_, NotebookState>,
    workspace_id: String,
    path: String,
) -> Result<tauri::ipc::Response, String> {
    api_path(&path)?;
    if !(path.ends_with("/audio") || path.ends_with("/download")) {
    ...
    let response = c
        .client
        .get(format!("{}{path}", c.base_url))
        .bearer_auth(&c.password)
        .send()
        .await
        .map_err(|_| "Media download failed".to_string())?;
    if !response.status().is_success() {
        return Err(format!("Media unavailable ({})", response.status()));
    }
    let bytes = response
        .bytes()
        .await
        .map_err(|_| "Media download interrupted".to_string())?;
    Ok(tauri::ipc::Response::new(bytes.to_vec()))
}

#[tauri::command]
pub async fn notebook_download(
    state: State<'_, NotebookState>,
    workspace_id: String,
    path: String,
    filename: String,
    ...
    if !response.status().is_success() {
        return Err(format!("Download unavailable ({})", response.status()));
    }
...
#[tauri::command]
pub async fn notebook_read_vault_note(
    state: State<'_, NotebookState>,
    workspace_id: String,
    ...
    {
        return Err("Invalid vault note path".into());
    }
    let path = fs::canonicalize(lease.0.vault.join(relative)).map_err(|e| e.to_string())?;
    if !path.starts_with(&lease.0.vault)
        || crate::engine::indexer::is_hidden(relative)
        || !matches!(
            path.extension().and_then(|x| x.to_str()),
            Some("md" | "markdown")
        )
    {
        return Err("Select a Markdown note inside this vault".into());
    }
...
mod tests {
    ...
    fn child_process_paths_are_never_verbatim() {
        assert_eq!(
            native_path(Path::new("/vault/.prism/notebook")),
            PathBuf::from("/vault/.prism/notebook")
        );
        #[cfg(windows)]
        {
            assert_eq!(
                native_path(Path::new(r"\\?\C:\vault\.prism\notebook")),
                PathBuf::from(r"C:\vault\.prism\notebook")
            );
            assert_eq!(
                native_path(Path::new(r"\\?\UNC\server\share\vault")),
...
#[tauri::command]
pub async fn notebook_import_copilot(
    app: tauri::AppHandle,
    state: State<'_, NotebookState>,
    workspace_id: String,
    provider: String,
) -> Result<Value, String> {
    let config = crate::config::load_runtime_config(&app)
        .ok_or("Configure the Prism provider first")?
        .omni_route;
    if provider != config.provider && provider != "openai_compatible" {
        return Err("Invalid import provider".into());
    }
    ...
    let credential_id = credential["id"]
        .as_str()
        .ok_or("Notebook returned no credential identifier")?;
