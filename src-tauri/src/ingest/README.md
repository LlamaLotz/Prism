...
## Dev runtime vs release runtime

- Dev: `python3 scripts/stage-dev-runtime.py` (also run by `npm run dev`)
  builds the worker with default features and stages
  `src-tauri/ingest-runtime/{bin/prism-ingest,manifest.json}` (gitignored).
  No third-party binaries or models are bundled; ffmpeg/yt-dlp resolve via
  PATH and uncompiled capabilities (`pdfium`/`ml`/`browser`) report
  `unavailable` so ingestion retries once with Python. Pass `--features`
  for a fuller local worker (needs CMake, toolchain, libclang).
  Parser-heavy pages run much faster optimized: use
  `python3 scripts/stage-dev-runtime.py --release` when web/PDF extraction
  feels slow under the debug worker.
- Release: `python3 scripts/build-ingest-runtime.py --release --assets-lock
  <lock.json>` (see below) stages the complete pinned runtime; only then use
  `tauri build --features ingest-rust --config src-tauri/tauri.ingest.conf.json`.
  The ordinary `tauri build` ships Python-only and the Rust engine reports
  unavailable.
...
Each source also has a 600s worker-side backstop: stages without their own
deadline (notably CPU-bound HTML parsing in unoptimized builds) fail with a
fallback-eligible `timeout` instead of stalling the panel silently. Python
fallback and legacy commands run unbuffered (`-u`/`PYTHONUNBUFFERED`) so progress
streams instead of arriving in one block on exit.
...
`wiki_index.html` / `wiki_a.html` / `wiki_b.html` model a long Wikipedia-style
article (nav chrome, prose sections, a 40-row station table, same-host links)
after a real East–West Line page that timed out under the legacy Crawl4AI
pipeline; the loopback tests pin table content, reading order, chrome
stripping, and a sub-10s extraction bound with no live-network dependency.
`wiki_nested.html` pins nested-table flattening: tables inside tables become
div/p (text kept in order) because the converter never finishes on deep
nesting — top-level tables still convert as real Markdown tables.
