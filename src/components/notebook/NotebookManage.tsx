...
import { NotebookClient, recordId, downloadBlob } from '../../services/notebook';
...
export function NotebookManage({ section, client, notebookId, config, onExport, onReference, onRestart }: {
  ...
  return <div className="nb-scroll nb-stack flex-1">
    ...
    {tab === 'episodes' && <>
      ...
      {episodes.map(e => <article key={e.id} className="nb-card nb-stack"><h3 className="font-semibold">{e.name}</h3><p className="nb-muted">{e.job_status ?? 'Ready'}</p>{e.error_message && <p className="nb-error">{e.error_message}</p>}<div className="nb-tabs">{(e.audio_file || e.audio_url) && <><Button disabled={busy} onClick={() => void run(async () => { const blob = await client.media(`/podcasts/episodes/${recordId(e.id)}/audio`, 'audio/mpeg'); setAudio({ id: e.id, url: URL.createObjectURL(blob) }); })}>Play</Button><Button disabled={busy} onClick={() => void run(async () => downloadBlob(await client.media(`/podcasts/episodes/${recordId(e.id)}/audio`, 'audio/mpeg'), `${e.name}.mp3`))}>Download audio</Button></>}<Button disabled={busy} onClick={() => void run(async () => { await client.request(`/podcasts/episodes/${recordId(e.id)}/retry`, 'POST'); await refresh(); })}>Retry</Button><Button onClick={() => deleteRow(`/podcasts/episodes/${recordId(e.id)}`, e.name)}>Delete</Button></div>{audio?.id === e.id && <audio controls autoPlay src={audio.url} className="w-full"/>}{e.transcript && <details><summary>Transcript</summary><Transcript value={e.transcript}/></details>}</article>)}
    </>}
