...
- Version snapshot / undo: every applied note write records via `record_note_version` (delta patch with 50-commit squash) before `sync_file`; `agent_undo_last{notePath}` restores the previous reconstructed version, writes it back, re-syncs, and returns its patch preview for display.
- Frontend: `AISidebar` adds Agent ON/OFF toggle (polls `agent_list_pending` every 3s + `knowledge-event` listener only when ON), a pending-approval panel (diff preview + Approve/Deny per pending), and "Undo last edit" affordance. When ON, user messages that parse as `{"tool","input"}` are dispatched through the Tool Bus; otherwise retrieval + chat continues as before.
...
Phase 2a is the read-only prerequisite; 2b activates the Tool Bus. The remaining phases follow the accepted order: structured documents (Phase 3); graph projections (Phase 4); Notebook consolidation (Phase 5); research (Phase 6); study (Phase 7); source-linked audio (Phase 8); and hardware/resource hardening (Phase 9).
